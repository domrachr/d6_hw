...
def index(request):
    ...
    books = Book.objects.all().prefetch_related('readers')
    biblio_data = {
...
def readers(request):
    ...
    readers = Reader.objects.all().prefetch_related('book_readers')
    readers_data = {
        'readers': [],
    }
    for reader in readers:
        # books = Book.objects.filter(readers__in = [reader]).distinct()
        books = Book.objects.filter(readers__in = [reader]).distinct()
        reader_data = {
            'reader': reader,
            'books': books,
        }
        readers_data['readers'].append(reader_data)
    return HttpResponse(template.render(readers_data, request))
...
def author_create_many(request):
    ...
    if request.method == 'POST':  # Наш обработчик будет обрабатывать и GET и POST запросы. POST запрос будет содержать в себе уже заполненные данные формы
        # Здесь мы заполняем формы формсета теми данными, которые пришли в запросе. Обратите внимание на параметр `prefix`. Мы можем иметь на странице не только несколько форм, но и разных формсетов, этот параметр позволяет их отличать в запросе.
